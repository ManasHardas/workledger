...
/**
 * Runtime dependencies the published package is allowed to declare. Empty today: everything the
 * CLI imports is inlined, so the tarball installs with nothing to resolve.
 *
 * Slot 7 (#10) breaks that: `better-sqlite3` is a native module and esbuild cannot inline a
 * `.node` binary. When it lands it must (1) stay in `EXTERNAL` below, (2) be added here *and* to
 * `packages/cli` `dependencies` as a real runtime dependency, (3) be added to
 * `pnpm-workspace.yaml` `allowBuilds` — it has a real postinstall, unlike esbuild — and (4) the
 * CI step that runs the packed CLI must `npm install --omit=dev` in the temp directory instead of
 * asserting `node_modules` is absent.
 */
export const EXPECTED_RUNTIME_DEPS = [];
...
async function main(argv) {
  ...
  console.log(`bundle-cli: ${path.relative(REPO_ROOT, OUTFILE)} — ${bytes} bytes, ${deps}`);
  return 0;
}
