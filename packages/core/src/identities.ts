/**
 * The `.workledger/identities.yaml` format and the resolution rule over it —
 ...
 * of it.
 *
 * Everything here is a pure function of a string, which is why it lives in `packages/core`: the
 * CLI reads the file with `node:fs` (`packages/cli/src/identities.ts`), the server reads it with
 * its own `readTextFile` (`packages/server/src/identities.ts`), and both hand the bytes to this
 * one parser. `GET /api/identities` and `workledger backlog show` therefore cannot disagree
 * about what the file says.
 *
 * Parsed by hand rather than with `yaml`, for the reason `packages/cli/src/config.ts` documents
 * at length: this module is reached from `commands/brief.ts`, which the `SessionStart` hook calls
 * inside a 300 ms budget, and the file is two keys deep. A file that does not parse is an *empty*
 * map rather than an error — "Missing file: emails display as before" is the contract, and a
 * malformed file must degrade to the same place rather than cost a session its brief.
 */
...
/**
 * The rows of an {@link IdentityMap} in the order `GET /api/identities` returns them: by
 * lower-cased email, so two servers over the same file answer byte-identically.
 */
export function identityList(identities: IdentityMap): Identity[] {
  return [...identities.keys()].sort().map((key) => identities.get(key) as Identity);
}
 ...
 */
export function resolveActor<T extends NamedActor>(actor: T, identities: IdentityMap): T {
...
export function resolveMaybe<T extends NamedActor>(
 ...
 */
export function formatActor(actor: NamedActor, identities: IdentityMap): string {
