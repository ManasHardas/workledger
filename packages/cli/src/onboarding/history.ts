 ...
 * the plan they consent to agree: the same inference over both stores (`./attribution.ts`,
 * amendment 10 — a session counts for every repo it is about, once each, wherever it started),
 * the same `filterSince` on file mtime (a session that was active yesterday is inside the 7-day
 * window however long ago it started). Both harnesses resume headlessly, so every session
 * counted here is one the resume plan can digest; `all` (amendment 9) has no cutoff.
 */
import { filterSince } from "../commands/backfill.js";
