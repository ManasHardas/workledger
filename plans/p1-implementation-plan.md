...
**Files:**
- Create: root `package.json` (private, `packageManager: pnpm@11` (the installed major; bump when the machine does)), `pnpm-workspace.yaml` (`packages/*`, `apps/*`), `tsconfig.base.json` (strict, `moduleResolution: node16` since the CLI runs unbundled in Node, ES2022), `vitest.workspace.ts`, `eslint.config.js`
