...
- **P8 (Onboarding and home) is the operator's priority (2026-09-09) and runs before P7 Wave 1:** spec `plans/feature-p8-onboarding-home.md`, contract `docs/contracts/p8/daemon-and-api.md`, tracking #75, builds #76 (daemon/multi-repo) → #77 (onboarding ops) and #78 (Home) → #79 (wizard) → #81 (QA e2e); #80 (release/brew) merged as #82. **The operator wants to walk through onboarding personally before `p8-shipped`; prepare a fresh-state run (`WORKLEDGER_HOME=$(mktemp -d) workledger`) and wait for the verdict.**
